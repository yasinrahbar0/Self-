...
SESSION_STRING = os.environ["SESSION_STRING"].strip()  # ✅ حتما strip
...
async def save_cmd(event):
    if not event.is_reply:
        return await event.reply("Reply to a media file ❌")
...
async def style_text(event):
    # فقط پیام‌های خودت و نه دستورات
    if event.out and not event.raw_text.startswith("."):
        text = event.raw_text
        if bold_mode:
            text = f"**{text}**"
        if italic_mode:
...
if __name__ == "__main__":
    # Flask و Telethon همزمان اجرا میشن
